...
void machine_setup(machine_t *m, int argc, char *argv[]) {
    ...
    m->state.gpr[sp] = stack + stack_size;
    m->state.gpr[sp] -= 8; // auxv
    m->state.gpr[sp] -= 8; // envp
    m->state.gpr[sp] -= 8; // argv end
    ...
    for (int i = args; i > 0; i--) {
        ...
        m->state.gpr[sp] -= 8; // arg[i]
        mmu_write(m->state.gpr[sp], (u8 *)&addr, sizeof(u64));
    }
    m->state.gpr[sp] -= 8; // argc
    mmu_write(m->state.gpr[sp], (u8 *)&args, sizeof(u64));
