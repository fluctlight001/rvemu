...
static void mmu_load_segment(mmu_t *mmu, elf64_phdr_t *phdr, int fd) {
    ...
    // mmap page aligned
    // mmap 是一个系统调用，用于将文件或者其他对象映射到进程的地址空间，以便能够直接在内存中对它进行访问。
    u64 addr = (u64)mmap((void *)aligned_vaddr, filesz, prot, MAP_PRIVATE | MAP_FIXED, fd, ROUNDDOWN(offset, page_size));
    assert(addr == aligned_vaddr);

    u64 remaining_bss = ROUNDUP(memsz, page_size) - ROUNDUP(filesz, page_size);
    if (remaining_bss > 0) {
        u64 addr = (u64)mmap((void *)(aligned_vaddr + ROUNDUP(filesz,page_size)),remaining_bss, prot, MAP_ANONYMOUS | MAP_PRIVATE | MAP_FIXED, -1, 0);
...
void mmu_load_elf(mmu_t *mmu, int fd) {
    ...
    FILE *file = fdopen(fd, "rb"); // r - read only ; b - binary
...
u64 mmu_alloc(mmu_t *mmu, i64 sz) {
    ...
    if (sz > 0 && mmu->alloc > TO_GUEST(mmu->host_alloc)) {
        if (mmap((void *)mmu->host_alloc,ROUNDUP(sz,page_size), PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0) == MAP_FAILED)
            fatal("mmap failed");
